...
namespace GameplanAPI.Shared.Abstractions.Handling
{
    public class Result
    {
        ...
        protected Result(bool isSuccess, IEnumerable<ValidationFailure> validationErrors)
        {
            if (isSuccess && validationErrors.Any() || !isSuccess && !validationErrors.Any())
            {
    ...
    public class Result<TValue> : Result
    {
        ...
        protected Result(TValue? value, bool isSuccess, IEnumerable<ValidationFailure> validationErrors) : base(isSuccess, validationErrors)
        {
            if (!isSuccess && value != null)
            {
                throw new ArgumentException("Invalid error", nameof(validationErrors));
